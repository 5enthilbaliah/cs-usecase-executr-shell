namespace UseCaseExecutR.Registration;
...
{
    public static void AddUseCaseExecutRClasses(IServiceCollection services, UseCaseExecutRServiceConfiguration configuration)
    {
        var assembliesToScan = configuration.AssembliesToRegister.Distinct().ToArray();

        ConnectImplementationsToTypesClosing(typeof(IUseCaseRunner<,>), services, assembliesToScan, false, configuration);
    }
    
    public static void AddRequiredServices(IServiceCollection services, UseCaseExecutRServiceConfiguration serviceConfiguration)
    {
        // Use TryAdd, so any existing ServiceFactory/IMediator registration doesn't get overridden
        services.TryAdd(new ServiceDescriptor(typeof(IExecutor), serviceConfiguration.MediatorImplementationType, serviceConfiguration.Lifetime));
    }
